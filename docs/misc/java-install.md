...
Installing Java is a critical first step to using or developing plugins for DivineMC. 
...
Those variants miss required dependencies for DivineMC. Therefore, using them is not recommended.
...
If you have started a DivineMC server with an incorrect Java version, Pterodactyl will automatically
